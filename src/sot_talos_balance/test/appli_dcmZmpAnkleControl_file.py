...
cm_conf.CTRL_MAX = 100.0  # temporary hack 100.0
...
# --- Phase --> needed for the dummy apparently (14.05)
#robot.phaseTrajGen = create_scalar_trajectory_generator(dt, 0.5, 'phaseTrajGen') # (dt, init_value, name) init_value pas coherente avec fichier
#robot.phaseScalar = Component_of_vector("phase_scalar")
#robot.phaseScalar.setIndex(0)
#robot.phaseTrajGen.playTrajectoryFile(folder + 'Phase.dat') # added
#plug(robot.phaseTrajGen.x, robot.phaseScalar.sin)

robot.phaseTrajGen = create_scalar_trajectory_generator(dt, 0.5, 'phaseTrajGen') # from appli_dcmZmpControl_distribute.py
robot.phaseScalar = Component_of_vector("phase_scalar")
robot.phaseScalar.setIndex(0)
robot.phaseTrajGen.playTrajectoryFile(folder + 'Phase.dat') # added
plug(robot.phaseTrajGen.x, robot.phaseScalar.sin)
...
# --- Rho --> needed for the dummy 15.05 = weight repartition of the robot on each leg
...
#robot.phaseTrajGen.playTrajectoryFile(folder + 'Rho.dat') # should not be imposed by .dat
...
#plug(robot.phaseScalar.sout, wp.phase)
...
# === 06.05.20 Test adding Wrench Distribution AND Ankle admittance controller following [Caron 2019] method
...
# --- Ankle roll & pitch => joint IDs checked 20.05
# right ankle pitch
RightPitchJoint = 16 # or 10

rightPitchSelec = Selec_of_vector("rightPitchSelec")
rightPitchSelec.selec(1, 2) # apparently simply extracts the pitch component of dRP from rightAnkleController
