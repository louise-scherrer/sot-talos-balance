...
# maintenant sans 's' a dynamics
from dynamic_graph.sot.dynamic_pinocchio import DynamicPinocchio # NOTE: IS IN /integration_tests/robotpkg-test-rc/install/lib/python 2.7/site-packages/dynamic_graph/sot/dynamics_pinocchio
from dynamic_graph.tracer_real_time import TracerRealTime
from rospkg import RosPack

#from dynamic_graph.ros import * # for rosImport()
...
sys.path.append("/local/lscherrer/lscherrer/sot_ws/install/lib/python2.7/site-packages/dynamic_graph/sot/") # new 09.04.20
from pattern_generator import PatternGenerator # this works, not very pretty but works for now
# END ADDED

#24.03.20 ajout pour rosImport(), je ne sais pas ce que ca fait en revanche
#ros = Ros(robot)
...
#    print(data)
    #robot.pg.velocitydes.value = (data.linear.x, data.linear.y, data.linear.z) # faire les modifs pour que cmd_vel soit un vecteur ou data.linear.x,y,z ?
...
# 20.04.20 NOW if CTRL_MAX is around 10.0, it goes over
...
#24.03.20
#ros.rosImport.add('Vector3', 'velocityDes', 'teleop_order') # ca ne marche pas, il ne connait pas 'ros'
#25.03.20
#plug(turtlebot_teleop.cmd_vel,robot.pg.velocityDes.value) # il ne connait pas 'turtlebot_teleop', normall cest pas un noeud de la sot
...
plug(robot.wp.waist, robot.keepWaist.featureDes.position) #avant wp.waistDes, marche pas non plus ...
...
#24.03.20 essai pour visualiser velocityDes
